...
document.getElementById('eliminar').addEventListener('click', function(event) { // Agrega un escuchador de eventos al botón 'Eliminar'
    event.preventDefault();
    var httpget = new XMLHttpRequest();
    httpget.open("DELETE", "http://localhost:4000/piramide", true);
    httpget.send();
    
    alert('Pirámide eliminada');
});

document.getElementById('lista-piramides').addEventListener('click', function(event) {
    event.preventDefault(); // Agrega un escuchador de eventos al botón 'Eliminar'
    window.location.href = "./piramides.html";
});
...
function generatePyramid(rowCount) { // Genera la pirámide
    ...
    var piramide = document.getElementById('container').innerHTML;
    var httpget = new XMLHttpRequest();
    ...
    httpget.onreadystatechange = function() {
        ...
        if (httpget.readyState == 4 && httpget.status == 200 || !httpget.responseText.startsWith('message')) {
            var piramideAnterior = httpget.responseText;
